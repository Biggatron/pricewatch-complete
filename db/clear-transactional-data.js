...
const PRESERVED_TABLES = ['app_config', 'user_account'];
...
async function main() {
  ...
  console.info('Clearing transactional database tables while preserving app_config and user_account');

  const transactionalTables = await getTransactionalTables();
  ...
  console.info(`Preserved tables: ${PRESERVED_TABLES.join(', ')}`);
  console.info(`Cleared tables: ${transactionalTables.join(', ')}`);
}

async function getTransactionalTables() {
